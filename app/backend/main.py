...
def glcm_features(patch, levels=16): # levels harus sama dengan training
    ...
    if patch.dtype.kind == 'f':
        ...
        # Jika input uint8 (0-255), scale down ke 0-15
        patch_scaled = (patch // (256 // levels)).astype(np.uint8)
...
def overlay_mask(original_image, mask, alpha=0.5):
    colored_mask = np.zeros((original_image.shape[0], original_image.shape[1], 3), dtype=np.uint8)
    ...
    # Set mask color to Red (R=255, G=0, B=0)
    colored_mask[:, :, 0] = 0   # R (OpenCV is BGR, so 2 is Red)
    colored_mask[:, :, 1] = 0   # G
    colored_mask[:, :, 2] = mask # B -> Di React tampil sbg Merah (RGB) atau Biru (BGR), sesuaikan nanti
    
    # Pastikan mask binary (0 atau 255)
    mask_bool = mask > 0
    
    # Blend hanya di area mask
...
def segment():
    ...
    try:
        ...
        # 4. Padding Image untuk Patching
        h, w = img_norm.shape
        half = PATCH_SIZE // 2
        img_padded = np.pad(img_norm, pad_width=half, mode='reflect')
        
        # 5. Ekstraksi Fitur per Piksel (Looping)
        print("Extracting features...")
        features = []
        
        # Loop ini mungkin agak lambat di Python murni
        for x in range(h):
            for y in range(w):
                # Extract features (akan menghasilkan 7 fitur)
                feats = extract_patch_features(img_padded, x, y, PATCH_SIZE)
                features.append(feats)
        
        features = np.array(features) # Shape: (16384, 7)
        
        # 6. Prediksi
        print("Predicting...")
        predictions = model.predict(features) # Output: (16384,)
        
        # 7. Reshape kembali ke bentuk gambar
        mask_pred = predictions.reshape(h, w).astype(np.uint8) * 255
        
        # 8. Post-Processing (Opsional: Bersihkan noise)
        kernel = np.ones((3,3), np.uint8)
        mask_clean = cv2.morphologyEx(mask_pred, cv2.MORPH_OPEN, kernel)
        
        # 9. Resize hasil kembali ke ukuran asli gambar upload (untuk display)
        orig_h, orig_w = img_original.shape
        mask_final = cv2.resize(mask_clean, (orig_w, orig_h), interpolation=cv2.INTER_NEAREST)
        ...
        return jsonify({
            "success": True,
            "data": {
                "mask": image_to_base64(mask_final),
